...
local function taskBar(difficulty,skillGapSent)
    Wait(100)
...
AddEventHandler(('__cfx_export_syn_minigame_%s'):format("taskBar"), function(setCB)
    setCB(taskBar)
end)

AddEventHandler(('__cfx_export_syn_minigame_%s'):format("closeGuiFail"), function(setCB)
    setCB(closeGuiFail)
end)